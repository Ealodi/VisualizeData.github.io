  ...
  export default {
    ...
    data() {
      return {
        ...
        sinC: '',
        cosC: '',
        originalColor: 'rgb(243, 84, 83, 0.12)',
        dataList: [
          {
            name: '页面卡顿\白屏',
            value: '1',
          ...
          },
          {
              "name": " 警方",
              "value": '102',
              "bgColor": "rgba(237,81,90,0.12)",
              "color": "rgb(210, 31.2, 33.6)"
          },
          {
            name: '闪退',
            value: '8',
        ...
        for (const mutation of mutationsList) {
          if (mutation.type === 'childList' && mutation.target === tooltipDiv) {
            ...
            d3.json("src/assets/json/ciyun.json").then(function(data) {
              var words = data[dates[0]][dates[1]];
              // 使用 self.newValues 而不是 this.newValues
              self.newValues = words;
              self.changeWords();
    ...
    methods:{
      ...
      generateRandomSimilarColor(color) {
        // Parse the original color
        const originalRGB = this.parseRGB(color);

        // Generate random variations for each channel
        const randomVariation = () => Math.floor(Math.random() * 21) - 10; // Adjust this range as needed
        const randomizedRGB = originalRGB.map(channel => Math.min(255, Math.max(0, channel + randomVariation())));

        // Convert back to CSS rgba format
        const rgbaColor = `rgba(${randomizedRGB.join(',')})`;
      ...
      getRandomBrightColor() {
        ...
        // 根据亮度调整颜色
        const adjustColor = value => (brightness > 128 ? value * 0.8 : value * 1.2);

        // 应用亮度调整并将RGB值转换为CSS颜色格式
