...
function loadData() {
  ...
  spinner = new Spinner(opts).spin(target); // END: loader spinner settings ****************************
...
function draw(data) {
  ...
  var ws = d3.layout.wordStream().size([width, height]).fontScale(d3.scale.linear()).minFontSize(globalMinFont).maxFontSize(globalMaxFont).data(data).flag(globalFlag) // .font(font)
  ...
  maxFreq = ws.maxFreq(); //set svg data.

  svg.transition().duration(300).attr({
    width: globalWidth,
    height: globalHeight
  });
  ...
  var dates = [];
  boxes.data.forEach(function (row) {
    dates.push(row.date);
  });
  ...
  styleAxis(axisNodes); //Display the vertical gridline
  ...
  styleGridlineNodes(gridlineNodes); //Main group

  mainGroup.attr('transform', 'translate(' + margins.left + ',' + margins.top + ')');
  var wordStreamG = mainGroup.append('g').attr("id", "wordStreamG"); // =============== Get BOUNDARY and LAYERPATH ===============
  ...
  var lenb = boundary.length; // Get the string for path

  var combined = lineCardinal(boundary.slice(0, lenb / 2)) + "L" + lineCardinal(boundary.slice(lenb / 2, lenb)).substring(1, lineCardinal(boundary.slice(lenb / 2, lenb)).length) + "Z"; // ============= Get LAYER PATH ==============

  layerPath = mainGroup.append("path").attr("d", combined).attr("visibility", "hidden").attr("class", "layerpath").attr({
    'fill-opacity': 1,
    'stroke-opacity': 0
  }); // draw curves
  ...
  curve.exit().remove();
  curve.enter().append('path').attr('d', area).style('fill', function (d, i) {
    return color(i);
  }).attr({
    "class": "curve",
    'fill-opacity': 0,
    stroke: 'black',
    'stroke-width': 0,
    topic: function topic(d, i) {
      return topics[i];
    }
  });
  curve.attr("d", area).style('fill', function (d, i) {
    return color(i);
  }).attr({
    'fill-opacity': 0,
    stroke: 'black',
  ...
  if (fileName.indexOf("Huffington") >= 0) {
    d3.json("data/linksHuff2012.json", function (error, rawLinks) {
      ...
      var visibleLinks = []; // select only links with: word place = true and have same id

      links.forEach(function (d) {
        var s = allWords.find(function (w) {
          return w.id === d.sourceID && w.placed === true;
        });
        var t = allWords.find(function (w) {
          return w.id === d.targetID && w.placed === true;
        });
      ...
      lineScale = d3.scale.linear().domain(d3.extent(visibleLinks, function (d) {
        return d.weight;
      })).range([0.5, 3]);
      opacScale = d3.scale.linear().domain(d3.extent(visibleLinks, function (d) {
        return d.weight;
      })).range([0.5, 1]);
      var connection = mainGroup.selectAll(".connection").data(visibleLinks, function (d) {
        return d.id;
      });
      connection.exit().remove();
      connection.enter().append("line").attr("class", "connection");
      connection.transition().duration(800).attr("opacity", isRel ? 1 : 0).attr({
        "x1": function x1(d) {
          return d.sourceX;
        },
        "y1": function y1(d) {
  ...
  function drawWords() {
    var prevColor;
    var texts = mainGroup.selectAll('.word').data(allWords, function (d) {
      return d.id;
    });
    texts.exit().remove();
    var textEnter = texts.enter().append('g').attr({
      transform: function transform(d) {
        return 'translate(' + d.x + ', ' + d.y + ')rotate(' + d.rotate + ')';
      }
    }).attr("class", "word").append('text');
    ...
    texts.transition().duration(800).attr({
      transform: function transform(d) {
        return 'translate(' + d.x + ', ' + d.y + ')rotate(' + d.rotate + ')';
      }
    }).select("text").attr('font-size', function (d) {
      return d.fontSize;
    }).attr({
      visibility: function visibility(d) {
        return d.placed ? "visible" : "hidden";
    ...
    mainGroup.selectAll(".connection").on("mouseover", function () {
      ...
      thisLink.style('cursor', 'crosshair'); // in order to select by byid, the id must not have space
    ...
    mainGroup.selectAll('.textData').on('mouseenter', function () {
      ...
      var allTexts = mainGroup.selectAll('.textData').filter(function (t) {
    ...
    mainGroup.selectAll('.textData').on('click', function () {
      ...
      var streamLayer = d3.select("path[topic='" + topic + "']")[0][0].__data__; //Push all points


      var points = Array(); //Initialize all points

      streamLayer.forEach(function (elm) {
        points.push({
          x: elm.x,
          y0: elm.y0 + elm.y,
          y: 0 //zero as default
      ...
      allTexts[0].forEach(function (t) {
        var data = t.__data__;
        var fontSize = data.fontSize; //The point

        var thePoint = points[data.timeStep + 1];
        ; //+1 since we added 1 to the first point and 1 to the last point.

        thePoint.y = -data.streamHeight; //Set it to visible.
        //Clone the nodes.
      ...
      wordStreamG.append('path').datum(points).attr('d', area).style('fill', prevColor).attr({
      ...
      var allOtherTexts = mainGroup.selectAll('.textData').filter(function (t) {
    ...
    topics.forEach(function (topic) {
      d3.select("path[topic='" + topic + "']").on('click', function () {
        mainGroup.selectAll('.textData').filter(function (t) {
          return t && !t.cloned && t.placed && t.topic === topic;
        }).attr({
          visibility: 'visible'
        }); //Remove the cloned element

        document.querySelectorAll("g[cloned='true'][topic='" + topic + "']").forEach(function (node) {
          node.parentNode.removeChild(node);
        }); //Remove the added path for it

        document.querySelectorAll("path[wordStream='true'][topic='" + topic + "']").forEach(function (node) {
          node.parentNode.removeChild(node);
        });
      });
    }); //Build the legends
    ...
    var legendNodes = legendGroup.selectAll('g').data(boxes.topics).enter().append('g').attr('transform', function (d, i) {
